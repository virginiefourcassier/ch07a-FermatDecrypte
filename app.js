...
(() => {
  ...
  class Entity {
    ...
    step(dt){
      ...
      // agitation thermique continue (évite une chute de vitesse quand la réaction s'arrête)
      // -> ajout de petites impulsions aléatoires proportionnelles à T
      const thermal = (18 * T); // intensité (px/s^2)
      this.vx += (Math.random()*2-1) * thermal * dt;
      this.vy += (Math.random()*2-1) * thermal * dt;
